...
#### ترقيات المخطط عند الديبلوي

الأعمدة المستجدّة على جدول `projects` تُضاف تلقائيًا عند أول اتصال بقاعدة البيانات بعد الديبلوي (استعلام تحقّق واحد عند إقلاع الخادم فقط، لا مع كل طلب)، لذا لا يلزم تشغيل `npm run db:init` بعد رفع التحديثات على Hostinger.

وإن احتجت تطبيقها يدويًا من phpMyAdmin فملفات الترقية موجودة في `api/migrations/` وهي آمنة للتكرار.
