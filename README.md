...
عند تشغيل `npm start` يقوم النظام أولًا بتنفيذ:

```bash
npm run db:init
```

وهذا ينشئ قاعدة البيانات إن كانت صلاحيات المستخدم تسمح، ثم ينشئ جداول `settings` و`projects`، ويضيف إعدادات الموقع والمشاريع الافتراضية إذا كانت القاعدة فارغة. يمكن تشغيل الأمر يدويًا عند الحاجة بعد ضبط متغيرات قاعدة البيانات.
